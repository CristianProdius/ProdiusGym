...
class UserProfile {
    ...
    func updateBMI() {
        if height > 0 && weight > 0 {
            ...
            debugLog("🧮 BMI CALC: height=\(height)cm, weight=\(weight)kg, heightInMeters=\(heightInMeters)m, BMI=\(bmi)")
        } else {
            bmi = 0.0
            debugLog("🧮 BMI CALC: Invalid data - height=\(height)cm, weight=\(weight)kg, BMI set to 0")
        }
        markAsUpdated()
    }
