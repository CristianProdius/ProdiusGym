# Terms of Service

**Last Updated: January 9, 2026**
**Effective Date: January 9, 2026**

---

## Important Legal Notice

PLEASE READ THESE TERMS CAREFULLY BEFORE USING GYMLY. BY ACCESSING OR USING THE SERVICE, YOU AGREE TO BE BOUND BY THESE TERMS. IF YOU DO NOT AGREE, DO NOT USE THE SERVICE.

---

## 1. Acceptance of Terms

These Terms of Service ("Terms") constitute a legally binding agreement between you ("you," "your," or "User") and ProdiusGym Fitness ("we," "us," or "Company") governing your use of the ProdiusGym mobile application and all related services (collectively, the "Service").

By downloading, installing, creating an account, or accessing any part of the Service, you acknowledge that you have read, understood, and agree to be bound by these Terms and our Privacy Policy.

---

## 2. Description of Service

ProdiusGym is a fitness tracking application designed for strength training and workout logging.

### Core Features (Free):
- Workout logging (exercises, sets, reps, weights)
- Exercise library and search
- Workout history and calendar
- Basic progress tracking
- Training split creation
- iCloud sync (optional)
- Apple Health integration (optional)

### Premium Features (Subscription Required):
- AI-powered workout summaries and insights
- Automatic personal record (PR) tracking
- Progress photo timeline with comparisons
- Pre-built workout templates
- Advanced streak analytics
- BMI tracking and analysis
- Unlimited workout history
- Custom app appearance and themes

### Device Requirements:
- iOS device running iOS 17.0 or later
- Apple ID for authentication
- Internet connection for sync features (optional for core functionality)

---

## 3. Eligibility

**You must be at least 13 years of age to use ProdiusGym** (or 16 in the European Union).

By accepting these Terms, you represent that:
- You have the legal capacity to enter into binding contracts
- You are not prohibited from using the Service under applicable laws
- All information you provide is accurate and truthful

---

## 4. Account Registration and Security

### Account Creation

To access certain features, you must create an account using Apple Sign In with valid Apple ID credentials.

### Your Responsibilities:
- Maintain confidentiality of your Apple ID credentials
- Use a strong password for your Apple ID
- Enable two-factor authentication
- Immediately notify us of any unauthorized access
- Not share your account with others

**We Are Not Liable for unauthorized access resulting from your failure to secure your account.**

---

## 5. Subscriptions and Payments

### Premium Subscription

ProdiusGym offers optional paid Premium subscriptions:
- **Monthly:** €2.99/month
- **Yearly:** €29.99/year (save 17%)

### Free Trial:
- 7-day free trial for new subscribers
- Full access to Premium features during trial
- Cancel anytime during trial to avoid charges
- Automatically converts to paid subscription after trial

### Payment Processing

All payments are processed through Apple's In-App Purchase system and are subject to Apple's terms and conditions.

**We Do Not:**
- Store your payment information
- Process payments directly
- Have access to your credit card details

### Auto-Renewal:
- Subscriptions automatically renew unless cancelled
- Charged to your Apple ID 24 hours before renewal date
- Current pricing applies at renewal

### How to Cancel:
1. iOS Settings → [Your Name] → Subscriptions
2. Select ProdiusGym
3. Tap "Cancel Subscription"

**Cancellation Terms:**
- Cancel anytime before next billing cycle
- Access to Premium features continues until end of current period
- No refunds for partial months

### Refunds

Refunds are handled through Apple's refund policy. To request a refund:
- Visit reportaproblem.apple.com
- Sign in with your Apple ID
- Select ProdiusGym purchase
- Choose "Request a Refund"

---

## 6. License and Access

Subject to these Terms, we grant you a limited, non-exclusive, non-transferable, revocable license to:
- Install and use ProdiusGym on your personal devices
- Access and use the Service for personal, non-commercial purposes
- Sync data across devices you own

### You May NOT:
- Copy, modify, or create derivative works of the app
- Reverse engineer, decompile, or disassemble the app
- Use the Service for commercial purposes without written permission
- Distribute the app outside the App Store
- Use automated systems (bots) to access the Service
- Interfere with or disrupt the Service

### Ownership

ProdiusGym Fitness owns all rights, title, and interest in:
- The ProdiusGym application
- Source code and design
- Trademarks, logos, and branding
- Content provided by us (templates, exercise data, etc.)

Your use of the Service does not grant you any ownership rights.

---

## 7. User Conduct and Acceptable Use

### You agree NOT to use the Service to:

**Illegal Activities:**
- Violate any local, state, national, or international law
- Infringe upon intellectual property rights
- Engage in fraudulent activities

**Harmful Content:**
- Upload viruses or malicious code
- Share content promoting violence or self-harm
- Post discriminatory or hateful content

**Service Abuse:**
- Attempt to crash our systems
- Scrape or harvest data without permission
- Create fake accounts or impersonate others
- Spam other users or our support team

---

## 8. User Content

### Your Content

"User Content" means any data you create, upload, or store in ProdiusGym:
- Workout logs and history
- Exercise notes and comments
- Progress photos
- Custom workout splits
- Profile information

### Ownership

**You Retain Ownership:**
- You own all User Content you create
- We do not claim ownership of your workouts, photos, or data
- You can export or delete your content anytime

### License to Us

By using the Service, you grant us a limited license to:
- Store and process your content
- Display your content back to you
- Sync your content across your devices (if enabled)
- Create backups for reliability

**This License Is:**
- Non-exclusive (you can use your data elsewhere)
- Revocable (terminated when you delete content)
- For the sole purpose of providing the Service

---

## 9. Medical and Fitness Disclaimers

**⚠️ IMPORTANT: PLEASE READ CAREFULLY ⚠️**

### Not Medical Advice

**GYMLY IS NOT A MEDICAL DEVICE AND DOES NOT PROVIDE MEDICAL ADVICE.**

The Service:
- Is for informational and tracking purposes only
- Does not diagnose, treat, or prevent any medical condition
- Is not a substitute for professional medical advice
- Should not be used for medical decision-making

**ALWAYS CONSULT:**
- Your physician before starting any exercise program
- A healthcare provider if you have medical conditions
- A doctor if you experience pain or injury

### Fitness and Exercise Risks

**EXERCISE INVOLVES INHERENT RISKS:**
- Injury (strains, sprains, tears, fractures)
- Cardiovascular events
- Aggravation of pre-existing conditions
- Other unforeseen health complications

**BY USING GYMLY, YOU:**
- Assume all risks associated with exercise
- Acknowledge you've consulted appropriate healthcare professionals
- Understand the importance of proper form and technique
- Agree to exercise within your abilities
- Will seek immediate medical attention if injured

### No Guarantees of Results

**WE DO NOT GUARANTEE:**
- Specific fitness outcomes or results
- Weight loss or muscle gain
- Improved health or performance
- Achievement of personal goals

**FITNESS RESULTS DEPEND ON:**
- Individual genetics and biology
- Consistency and effort
- Nutrition and recovery
- Proper technique
- Many other factors outside our control

### User Responsibility

**YOU ARE RESPONSIBLE FOR:**
- Using proper exercise form and technique
- Choosing appropriate weights and intensity
- Warming up and cooling down
- Listening to your body
- Stopping if you experience pain or discomfort
- Seeking qualified coaching if needed

### Age-Specific Warnings

**Youth (Under 18):**
- Should train under adult supervision
- Should focus on proper technique over heavy weights
- Consult pediatrician before starting strength training

**Seniors (Over 65):**
- Should consult doctor before beginning programs
- May need modified exercises
- Should focus on safety and balance

**Pregnant or Postpartum:**
- MUST consult doctor before exercising
- Need specialized programming
- Should avoid certain exercises

### Pre-Existing Conditions

**IF YOU HAVE:**
- Heart disease or cardiovascular conditions
- High blood pressure
- Diabetes
- Respiratory conditions
- Joint problems or arthritis
- Recent surgery or injury
- Any chronic health condition

**YOU MUST:**
- Consult your doctor before using ProdiusGym
- Follow all medical advice and restrictions
- Modify exercises as recommended by healthcare providers
- Stop exercising if symptoms occur

---

## 10. Disclaimers and Limitations of Liability

**⚠️ IMPORTANT LEGAL PROTECTIONS ⚠️**

### Service Provided "AS IS"

THE SERVICE IS PROVIDED "AS IS" AND "AS AVAILABLE" WITHOUT WARRANTIES OF ANY KIND, EITHER EXPRESS OR IMPLIED.

We Disclaim Warranties Of:
- MERCHANTABILITY
- FITNESS FOR A PARTICULAR PURPOSE
- ACCURACY OR COMPLETENESS
- ERROR-FREE OPERATION

### Limitation of Liability

**TO THE MAXIMUM EXTENT PERMITTED BY LAW:**

ProdiusGym Fitness SHALL NOT BE LIABLE FOR ANY INDIRECT, INCIDENTAL, SPECIAL, CONSEQUENTIAL, OR PUNITIVE DAMAGES, INCLUDING:
- Loss of data or content
- Personal injury or property damage
- Emotional distress
- Business interruption

ARISING FROM:
- Use or inability to use the Service
- Reliance on any content in the Service
- Errors or omissions in the Service
- Unauthorized access to your data

**WHETHER BASED ON CONTRACT, TORT, NEGLIGENCE, OR ANY OTHER LEGAL THEORY, EVEN IF WE HAVE BEEN ADVISED OF THE POSSIBILITY OF SUCH DAMAGES.**

### Cap on Liability

TO THE MAXIMUM EXTENT PERMITTED BY LAW, OUR TOTAL LIABILITY FOR ALL CLAIMS SHALL NOT EXCEED THE GREATER OF:
- Amount you paid in the 12 months before the claim
- €100 (one hundred euros)

---

## 11. Term and Termination

### Termination by You

You may terminate at any time by:
- Deleting your account in Settings → Account → Delete Account
- Emailing sebastian.kucera@icloud.com to request account deletion

**Effect:** Access to Service immediately ends; Premium features stop; data deleted within 30 days.

### Termination by Us

We may suspend or terminate your account immediately for:
- Violation of these Terms
- Violation of laws or regulations
- Fraudulent or abusive behavior
- Non-payment of subscription fees

---

## 12. Changes to Service and Terms

We reserve the right to:
- Modify, suspend, or discontinue any part of the Service
- Change features or functionality
- Update these Terms

**Notification:**
- Material changes: Email + in-app notification
- 30 days notice before changes take effect

**Your Options:**
- Continue using = acceptance of new Terms
- Stop using = disagreement with new Terms

---

## 13. Dispute Resolution

### Informal Resolution

Before filing legal action, you must:
- Contact us at sebastian.kucera@icloud.com
- Describe your dispute in detail
- Provide 30 days for us to respond and resolve

### Governing Law

These Terms are governed by the laws of the United States and the State of California, without regard to conflict of law principles.

### Jurisdiction

For legal proceedings:
- Exclusive jurisdiction: Courts of California, United States
- You consent to personal jurisdiction
- Small claims court actions may be brought in your local jurisdiction

---

## 14. General Provisions

### Entire Agreement

These Terms, along with our Privacy Policy, constitute the entire agreement between you and ProdiusGym Fitness regarding the Service.

### Severability

If any provision of these Terms is found invalid or unenforceable, remaining provisions remain in full effect.

### Assignment

You may not transfer or assign your rights. We may assign these Terms freely.

### No Waiver

Our failure to enforce any right does not constitute a waiver.

---

## 15. Contact Information

**Questions About These Terms?**

**General Inquiries:**
Email: sebastian.kucera@icloud.com
Subject: "Terms of Service Question"

**Support:**
Email: sebastian.kucera@icloud.com

**Mailing Address:**
ProdiusGym Fitness
Attn: Legal Department
[Your Address Here]

---

## Acknowledgment and Acceptance

BY USING GYMLY, YOU ACKNOWLEDGE THAT:

✓ You have read and understood these Terms of Service
✓ You have read and understood our Privacy Policy
✓ You agree to be bound by these Terms
✓ You understand the disclaimers and limitations of liability
✓ You assume all risks associated with exercise and training
✓ You will consult appropriate healthcare professionals before exercising
✓ You are at least 13 years old (or age of majority in your jurisdiction)
✓ You have the authority to enter into this agreement

**If you do not agree to these Terms, do not use ProdiusGym.**

---

*Thank you for using ProdiusGym. Train hard, train smart, and stay safe! 💪*

*Last reviewed and updated: November 13, 2024*
